...
public class FindWords212 {
    ...
    public List<String> findWords(char[][] board, String[] words) {
        if (words.length == 0) {
            ...
            boolean[][] forbidden = new boolean[board.length][board[0].length];
            Tree root = new Tree();
            ...
            for (int i = 0; i < board.length; i++) {
                for (int j = 0; j < board[i].length; j++) {
                    dfs(board, i, j, root, result, forbidden);
                }
            }
            return result;
        }
    ...
    public void dfs(char[][] board, int x, int y, Tree root, List<String> result, boolean[][] forbidden) {
        if (forbidden[x][y]) {
            return;
        }
        int index = board[x][y] - 'a';
        if (root.children[index] != null) {
            Tree node = root.children[index];
            if (node.word != null && !result.contains(node.word)) {
            ...
            for (int[] p : position) {
                ...
                if (nextX >= 0 && nextY >= 0 && nextX < board.length && nextY < board[0].length) {
                    forbidden[x][y] = true;
                    dfs(board, nextX, nextY, node, result, forbidden);
                    forbidden[x][y] = false;
                }
            }
        }
    ...
    public void insert(String word, Tree root) {
